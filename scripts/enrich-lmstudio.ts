...
function familyRepresentatives(plugins: DiscoveryPlugin[]): DiscoveryPlugin[] {
  const families = new Map<string, DiscoveryPlugin>();
  for (const plugin of plugins) {
    const current = families.get(plugin.family);
    if (!current || plugin.score > current.score) families.set(plugin.family, plugin);
  }
...
function meaningfulStrings(value: unknown, path = "root", output: Array<{ path: string; value: string }> = []) {
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => meaningfulStrings(item, `${path}[${index}]`, output));
    return output;
  }

  if (value && typeof value === "object") {
    for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
      if (/^(?:url|artifactUrl|avatarUrl|downloadUrl)$/i.test(key)) continue;
...
async function fetchDetail(identifier: string): Promise<{ endpoint: string; data: unknown | null }> {
  const candidates = [`${HUB_API}/${identifier}`, `${HUB_API}/${identifier}/`];
  for (const endpoint of candidates) {
    try {
      const response = await fetch(endpoint, {
        headers: {
          Accept: "application/json",
          "User-Agent": "local-ai-tools-enrichment/0.1 (+https://github.com/sahansera/local-ai-tools)",
        },
      });
      if (!response.ok) continue;
      return { endpoint, data: await response.json() };
    } catch {
      // Review-first enrichment: a failed detail request should not abort the entire snapshot.
    }
  }
  return { endpoint: candidates[0], data: null };
}

async function enrichPlugin(plugin: DiscoveryPlugin): Promise<EnrichedPlugin> {
  const detail = await fetchDetail(plugin.identifier);
  const strings = detail.data ? meaningfulStrings(detail.data) : [];
  const sourceFields = [...new Set(strings.map((item) => item.path))].slice(0, 100);
  const detailText = strings.map((item) => item.value).join("\n");
  const evidenceText = [plugin.name, plugin.identifier, plugin.description, detailText]
    .filter(Boolean)
    .join("\n")
    .slice(0, 250_000);
...
async function main(): Promise<void> {
  const discovery = JSON.parse(await readFile(resolve(DISCOVERY_PATH), "utf8")) as DiscoverySnapshot;
  if (!Array.isArray(discovery.plugins)) {
    throw new Error(`Invalid ${DISCOVERY_PATH}; run pnpm discover:lmstudio first.`);
  }

  const representatives = familyRepresentatives(discovery.plugins);
  const limit = parseLimit();
  const selected = Number.isFinite(limit) ? representatives.slice(0, limit) : representatives;

  console.log(`Enriching ${selected.length} LM Studio plugin families (of ${representatives.length})...`);
  const plugins = await mapConcurrent(selected, CONCURRENCY, enrichPlugin);
  ...
  for (const plugin of plugins) {
    ...
    for (const capability of Object.keys(plugin.enrichment.capabilities)) {
  ...
  console.log(`Fetched detail JSON for ${detailFetched}/${plugins.length} plugins.`);
  console.log(`Wrote ${OUTPUT_PATH}.`);
  console.log("\nCapability counts:");
  for (const [name, count] of Object.entries(capabilityCounts).sort((a, b) => b[1] - a[1])) {
    console.log(`${String(count).padStart(3)}  ${name}`);
  }
  console.log("\nRisk signals:");
  for (const [name, count] of Object.entries(riskCounts).sort((a, b) => b[1] - a[1])) {
    console.log(`${String(count).padStart(3)}  ${name}`);
  }
}
