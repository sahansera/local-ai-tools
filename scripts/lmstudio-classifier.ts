...
const CAPABILITY_RULES: Rule[] = [
  ...
  },
  {
    ...
    patterns: [
      /context (?:compact|compress|compression|window)/i,
      /\bcompactor\b/i,
      /\bcompressor\b/i,
      /token (?:saving|reduction|budget)/i,
    ],
    confidence: "high",
  },
  {
  ...
  },
  {
    ...
    patterns: [
      ...
      /time zone/i,
      /timezone/i,
      /inject(?:s|ing)? (?:the )?(?:current )?time/i,
    ],
    confidence: "high",
  },
  {
...
export function classifyPlugin(text: string): Classification {
  ...
  const explicitNetwork = explicitBoolean(
    normalizedText,
    [
      /requires? (?:an )?(?:internet|network) connection/i,
      /requires? network/i,
    ],
    [
      /no network/i,
  ...
  const networkRequired =
    explicitNetwork !== "unknown"
      ? explicitNetwork
      : risks["network-access"]?.length
        ? true
        : "unknown";
  ...
  const explicitApiKey = explicitBoolean(
    ...
    [
      /no api[- ]?key/i,
      /without (?:an )?api[- ]?key/i,
      /does not require (?:an )?api[- ]?key/i,
    ],
  );
  const apiKeyRequired =
    explicitApiKey !== "unknown"
      ? explicitApiKey
      : risks.credentials?.length
        ? true
        : "unknown";
...
export function searchableTerms(classification: Classification): string[] {
  return [
    ...Object.keys(classification.capabilities),
    ...Object.keys(classification.risks),
  ];
}
