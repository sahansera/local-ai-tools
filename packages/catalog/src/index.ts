...
export async function loadTools(): Promise<Tool[]> {
  const files = (await readdir(dataDir)).filter((file) => /\.ya?ml$/i.test(file)).sort();
  const tools: Tool[] = [];
  ...
  for (const file of files) {
    ...
    if (ids.has(parsed.id)) throw new Error(`Duplicate tool id: ${parsed.id}`);
    ids.add(parsed.id);
    tools.push(parsed);
  }
...
export async function generateCatalog(): Promise<void> {
  ...
  await writeFile(path.join(generatedDir, 'catalog.json'), `${JSON.stringify(payload, null, 2)}\n`);
}
